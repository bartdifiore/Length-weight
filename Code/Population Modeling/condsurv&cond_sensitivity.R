# -----------------------------------------------------------------------------
# Sensitivity Analysis 4: Condition Affects Survival & Condition (2 way)
# If we assume condition impacts survival, how do the spawning stock abundance
# and total population change when we vary condition?
# -----------------------------------------------------------------------------
library(tidyverse)
...
for(j in 1:length(cond_sens)){
  ...
  for(i in 2:101){
    ...
    # add in survival probability change due to condition (sensitivity)
    ...
    # condition and total eggs function (sensitivity)
...
tibble(cond_s=cond_sens,ssa=ssa_res)%>%
  ...
  geom_col()+
  labs(title="Condition Two-way Sensitivity Analysis: SSA",
       x="Condition Sensitivity Factor",
       y="Spawning Stock Abundance (SSA)")+
  theme_bw()

# calculate total population at time step 100
pop_res<-c()
for(i in 1:10){
  sens_res[[i]]%>%
    filter(timestep==100)%>%
...
tibble(cond_s=cond_sens,pop=pop_res)%>%
  ...
  geom_col()+
  labs(title="Condition Two-way Sensitivity Analysis: Total Population",
       x="Condition Sensitivity Factor",
       y="Total Population")+
  theme_bw()
